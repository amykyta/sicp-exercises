...
(define (repeated g n)
  (define (repeated-iter i)
    (if (>= i n)
        g
        (compose g (repeated-iter (inc i)))))
        ;(repeated-iter (lambda (x) (g (f x))) (inc i))))
  (repeated-iter 1))

(define (smooth f)
  (let ((average (lambda (a b c) 
                   (/ (+ a b c) 
                      3.0)))
        (dx 0.01))
    (lambda (x) 
...
(define (print-range f a b dx)
  (define (iter x)
    (cond ((>= x b) (display (f b)) (newline))
          (else (display (f x)) (newline) (iter (+ x dx)))))
  (iter a))
;(print-range sin 0 10 0.3)
;(newline)
;(print-range (smooth sin) 0 10 0.3)
;(newline)
;(print-range (repeated (smooth sin) 100) 0 10 0.3)
...
(define (fixed-pointv2 f first-guess)
  ((iterative-improve (lambda (guess)
                        (< (abs (- guess (f guess))) tolerance))
                      f)
   first-guess))

(fixed-pointv2 (average-damp (lambda (y) (/ 4 y))) 1.0)
